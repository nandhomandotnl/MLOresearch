...
int main(int argc, char* argv[]){
  ...
  uint32_t nAPInf = 1; // number of Access point
  uint32_t nWifiInf = 2; // number of stations
  ...
  NodeContainer apInf;
  apInf.Create(nAP);
  NodeContainer staInf;
  staInf.Create(nWifi);
  
  NetDeviceContainer apdevicesInf;
  NetDeviceContainer stadevicesInf;
  ...
  Ssid ssid2 = Ssid("ns3-interference");
  ...
  for (uint32_t i=0; i<nWifi; ++i){
    ...
    for (uint32_t i=0; i<nWifiInf; ++i){
      ...
      // Link 1: 5GHz
      phy.Set(0, "ChannelSettings", StringValue("{0, 80, BAND_6GHZ, 0}"));
      wifi.SetRemoteStationManager(2, "ns3::ConstantRateWifiManager",
                      "DataMode", StringValue("EhtMcs7"),
                      "ControlMode", StringValue("HeMcs4"));
                    
      stadevicesInf.Add(wifi.Install(phy, wifiMac, staInf.Get(i)));
  }
  ...
  for (uint32_t i=0; i<nAP; ++i){
     ...
     phy.EnablePcap("cap", apdevices.Get(0), 0);
     
       // setup AP
       
  for (uint32_t i=0; i<nAPInf; ++i){
      wifiMac.SetType("ns3::ApWifiMac",
                  "Ssid", SsidValue(ssid2),
                  "QosSupported", BooleanValue(true));
      apdevicesInf.Add(wifi.Install(phy, wifiMac, apInf.Get(i)));
  }
  ...
  mobility.Install(ap); 
  mobility.Install(staInf);
  mobility.Install(apInf); 
  ...
  stack.Install(sta);
  stack.Install(apInf);
  stack.Install(staInf);
  Ipv4AddressHelper address;
  address.SetBase("10.1.0.0", "255.255.255.0");
  Ipv4InterfaceContainer wifiApInterfaceInf = address.Assign(apdevicesInf);
  Ipv4InterfaceContainer wifiStaInterfacesInf = address.Assign(stadevicesInf);
  ...
  for (std::size_t i=0; i<nWifi; ++i){
    ...
    for (std::size_t i=0; i<nWifiInf; ++i){
      serverInterfaces.Add(wifiApInterfaceInf.Get(0));
      clientNodes.Add(staInf.Get(i));
  }
  ...
  fname << "MLO/flowmon-data-sta-" << nWifi << ".xml";
  std::cout << fname.str();
